...
if [ -f "$TARGET_PATH" ]; then
  ...
  cat > "$TARGET_PATH" << 'EOF'
# (Full patched Python code — same as original you gave)
import platform
import subprocess
import sys
...
GRPO_PATH="$HOME/rl-swarm/.venv/lib/python3.12/site-packages/genrl/trainer/grpo_trainer.py"
if [ -f "$GRPO_PATH" ]; then
...
SWARM_PEM_PATH="$HOME/rl-swarm/swarm.pem"
...
cd "$HOME/rl-swarm" || { echo "❌ rl-swarm directory not found!"; exit 1; }
